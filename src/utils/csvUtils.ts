...
export function exportCarsToCsv(cars: CarAgent[]): void {
    const csvContent = [
        ["id", "type", "longitude", "latitude", "speed", "maxSpeed", "targetSpeed", "route"],
        ...cars.map((car) => [
            car.id,
            car.carType.id,
            car.position[0],
            ...
            car.maxSpeed,
            car.targetSpeed,
            `"${JSON.stringify(car.route).replace(/"/g, '""')}"`, // <- aquí
        ]),
    ]
        .map((row) => row.join(","))
        .join("\n");
...
export function importCarsFromCsv(
    ...
    Papa.parse(file, {
        ...
        skipEmptyLines: true,
        complete: (results) => {
            const parsedCars = (results.data as any[]).map((data) => {
            ...
            // ✅ Ahora ya puedes usar `parsedCars`
